...
//  Products(this.authToken, this.userId, this._items);
  ...
  List<Log> get recentTransactions {
  ...
  List<Log> get dayrecentTransactions {
    return _history.where((tx) {
      return tx.time.day == DateTime.now().day;
    }).toList();
  }

  List<Log> get monthrecentTransactions {
    return _history.where((tx) {
      return tx.time.isAfter(
        DateTime.now().subtract(
          Duration(days: 30),
  ...
  Future<void> fetchAndSetProducts() async {
    ...
    try {
      ...
      print(extractedData);
      final List<Log> loadedProducts = [];
      extractedData.forEach((prodId, prodData) {
        loadedProducts.add(Log(
          id: prodId,
          name: prodData['name'],
          amount: prodData['amount'],
          happn: prodData['happn'],
          time: DateTime.parse(prodData['time']),
          price: prodData['price'],
        ));
      });
      _history = loadedProducts.reversed.toList();
      notifyListeners();
    } catch (error) {
      throw (error);
    }
